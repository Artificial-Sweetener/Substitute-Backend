...
"""PromptServer websocket publisher for model catalog change events."""
...
class PromptServerPublisherLike(Protocol):
    """Subset of Comfy PromptServer needed for websocket publication."""

    def send_sync(self, event: str, data: object, sid: str | None = None) -> None:
        """Queue a websocket event for Comfy clients."""


class PromptServerModelCatalogPublisher:
    """Publish model catalog changes through Comfy PromptServer."""
    ...
    def publish(self, event: ModelCatalogChangeSet) -> None:
        """Publish a model catalog event and swallow PromptServer failures."""

        send_sync = getattr(self._prompt_server, "send_sync", None)
        if not callable(send_sync):
            self._logger.debug("Model catalog event skipped; PromptServer has no send_sync")
            return
        try:
            send_sync(EVENT_TYPE, event.to_payload(), None)
        except Exception:
            self._logger.exception(
                "Failed to publish model catalog change event",
                extra={
...
__all__ = ["PromptServerModelCatalogPublisher"]
