...
"""PromptServer websocket publisher for model-loading telemetry."""
...
from substitute_backend.features.model_loading.domain.events import ModelLoadProgressEvent

EVENT_TYPE = "substitute_model_load_progress"


class PromptServerPublisherLike(Protocol):
    """Subset of Comfy PromptServer needed for telemetry publication."""
...
class PromptServerModelLoadPublisher:
    """Publish model-loading telemetry through Comfy PromptServer."""
    ...
    def publish(self, event: ModelLoadProgressEvent) -> None:
        ...
        if not callable(send_sync):
            self._logger.debug("Model-load telemetry skipped; PromptServer has no send_sync")
            return
        ...
        try:
            ...
            self._logger.exception("Failed to publish model-load telemetry")
